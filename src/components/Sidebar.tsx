...
import {
  LayoutDashboard, FileText, Database, 
  CheckCircle, Shield, Archive, TreePine, 
  Settings, Layers, Plus, Users, BarChart2,
  UserCog, Map, ClipboardList, FileCheck,
  PenTool, Eye
} from "lucide-react";
import { hasPermission, hasAnyPermission, getRoleDisplayName, PERMISSIONS, type UserRole } from "@/lib/auth";
...
export default function Sidebar({ user }: SidebarProps) {
  ...
  const buildNavGroups = () => {
    ...
    if (user.role === 'lapangan') {
      manajemenItems.push(
        { name: "Daftar Klem", path: "/rtt/klem", icon: <ClipboardList size={18} /> },
        { name: "Rekap Klem", path: "/rtt/rekap-klem", icon: <BarChart2 size={18} /> },
        { name: "Berita Acara", path: "/rtt/berita-acara", icon: <FileCheck size={18} /> },
      );
    }
    ...
    if (user.role === 'admin') {
      manajemenItems.push(
        { name: "RTT Summary", path: "/rtt/summary", icon: <FileText size={18} /> },
    ...
    if (hasPermission(user, PERMISSIONS.DOCUMENT_REVIEW) || 
    ...
    if (hasPermission(user, PERMISSIONS.REPORT_VIEW)) {
      laporanItems.push(
        { name: "Laporan RTT", path: "/reports", icon: <BarChart2 size={18} /> },
    ...
    if (user.role === 'sysadmin') {
      groups.push({
        title: "Administrasi",
        items: [
          { name: "Kelola User", path: "/admin/users", icon: <UserCog size={18} /> },
  ...
  return (
    <aside className="glass-sidebar w-[260px] flex flex-col h-screen shrink-0 relative z-50">
      ...
      <div className="h-[72px] flex items-center px-7 gap-3.5 shrink-0 border-b border-white/[0.04]">
        ...
        <div>
          <h2 className="text-[13px] font-extrabold text-white tracking-wide">PERHUTANI</h2>
          <p className="text-[9px] text-slate-500 font-medium tracking-wider">Divre Jabar & Banten</p>
        </div>
      </div>
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`${isActive ? "text-emerald-400" : "text-slate-500 group-hover:text-slate-400"} transition-colors`}>
                      {item.icon}
                    </div>
                    <span className={`text-[13px] font-semibold ${isActive ? "text-white" : "text-inherit"}`}>
                      {item.name}
                    </span>
                  </Link>
                );
      ...
      <div className="p-4 border-t border-white/[0.04] shrink-0">
        <div className="glass-card rounded-xl p-3.5 flex items-center gap-3">
          <div className="shrink-0 w-9 h-9 rounded-full bg-gradient-to-br from-emerald-500/20 to-teal-500/20 flex items-center justify-center text-[11px] font-bold text-emerald-400 border border-emerald-500/20">
            {user?.nama?.substring(0,2).toUpperCase() || user?.username?.substring(0,2).toUpperCase()}
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-[12px] font-semibold text-slate-200 truncate">{user?.nama || user?.username}</p>
            <p className="text-[10px] text-slate-500 font-medium truncate">{getRoleDisplayName(user?.role)}</p>
          </div>
