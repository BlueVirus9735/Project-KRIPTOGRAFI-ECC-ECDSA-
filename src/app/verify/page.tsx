...
import { Shield, CheckCircle, XCircle, Upload, FileKey, ArrowLeft } from "lucide-react";
...
function VerifyContent() {
  ...
  return (
    <div className="max-w-[1100px] mx-auto space-y-6 animate-fade-in relative z-10">
      <div className="pb-6 border-b border-white/[0.04] flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">Verifikasi Digital (Publik)</h1>
          <p className="text-slate-500 text-[13px] mt-1 font-medium">
            Verifikasi keaslian data RTT menggunakan Kriptografi ECDSA
          </p>
        </div>
        <button 
          onClick={() => router.push('/login')}
          className="flex items-center gap-2 bg-slate-800/50 hover:bg-slate-800 text-slate-300 px-4 py-2.5 rounded-xl transition-colors font-medium text-sm border border-slate-700/50"
        >
...
export default function VerifyPage() {
  return (
    <div className="min-h-screen bg-[#020617] p-8 relative overflow-hidden font-sans">
      <div className="absolute inset-0 bg-[url('/pexels-lauripoldre-36099638.jpg')] bg-cover bg-center bg-no-repeat opacity-5 z-0" />
      <div className="absolute top-[-20%] left-[-10%] w-[800px] h-[800px] bg-emerald-500/10 rounded-full blur-[120px] mix-blend-screen pointer-events-none z-0" />
      <VerifyContent />
    </div>
  );
}
