    ...
    <div className="max-w-[900px] mx-auto space-y-6 animate-fade-in">
      <div className="pb-6 border-b border-white/[0.04]">
        <h1 className="text-xl font-bold text-white flex items-center gap-2">
          <ShieldCheck className="text-emerald-400" /> Pengesahan Final RTT — Kepala Divisi
        </h1>
      ...
      {!successData ? (
        <div className="space-y-5">
          ...
          <div className="glass-card p-7">
            <h3 className="text-white font-bold text-[15px] mb-4 flex items-center gap-2">
              <FileText size={16} className="text-blue-400" /> 1. Pilih Dokumen Menunggu Pengesahan
            </h3>
            <select
              value={selectedRtt}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-white font-bold text-[15px] flex items-center gap-2">
                  <Lock size={16} className="text-amber-400" /> 2. Buka dan Periksa Isi Dokumen
                </h3>
                {encContent.has_kph_signature && (
                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                    <Shield size={11} className="text-emerald-400" />
                    <span className="text-[10px] text-emerald-400 font-bold">SIGNED KPH + PHW</span>
                  </div>
                )}
              </div>
          ...
          {decryptedDocData && selectedRtt && (
            <div className="glass-card p-7 border border-emerald-500/20 bg-emerald-500/5 animate-slide-up">
              <h3 className="text-white font-bold text-[15px] mb-3 flex items-center gap-2">
                <Key size={16} className="text-amber-400" /> 3. Sahkan dengan Tanda Tangan Digital
              </h3>
              ...
              <div className="bg-slate-900/50 rounded-xl px-4 py-3 mb-5 space-y-2">
                <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mb-2">Rantai Tanda Tangan:</p>
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${encContent?.has_kph_signature ? "bg-emerald-400" : "bg-slate-600"}`} />
                  <span className="text-[12px] text-slate-300">ECDSA KPH: {encContent?.has_kph_signature ? "✅ Tersedia" : "⏳ Belum ada"}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${encContent?.has_phw_signature ? "bg-emerald-400" : "bg-slate-600"}`} />
                  <span className="text-[12px] text-slate-300">ECDSA PHW: {encContent?.has_phw_signature ? "✅ Tersedia" : "⏳ Belum ada"}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
                  <span className="text-[12px] text-amber-300 font-medium">ECDSA Kepala Divisi: ⏳ Menunggu pengesahan</span>
                </div>
              </div>
        ...
        <div className="glass-card p-8 text-center animate-slide-up border-emerald-500/30 bg-emerald-500/5">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Dokumen Berhasil Disahkan!</h2>
          <p className="text-emerald-400 font-medium text-[13px] mb-8">{successData.message}</p>

          <div className="bg-slate-900/50 rounded-xl p-5 text-left space-y-4 border border-white/[0.05]">
            <div>
              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Hash SHA-256 Final</p>
              <p className="font-mono text-emerald-300 text-[12px] break-all">{successData.hash}</p>
            </div>
            <div>
              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Signature ECDSA Kepala Divisi</p>
              <p className="font-mono text-blue-300 text-[12px] break-all">{successData.signature}</p>
            </div>
            <div className="flex gap-4">
              <div className="flex-1 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3 py-2 text-center">
                <p className="text-[10px] text-slate-500 mb-1">Sig. KPH</p>
                <p className="text-[12px] font-bold text-emerald-400">{successData.kph_sig_status === "valid" ? "✅ VALID" : successData.kph_sig_status}</p>
              </div>
              <div className="flex-1 bg-blue-500/10 border border-blue-500/20 rounded-lg px-3 py-2 text-center">
                <p className="text-[10px] text-slate-500 mb-1">Sig. PHW</p>
                <p className="text-[12px] font-bold text-blue-400">{successData.phw_sig_status === "valid" ? "✅ VALID" : successData.phw_sig_status}</p>
              </div>
            </div>
          </div>
