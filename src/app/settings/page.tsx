    ...
    <div className="max-w-[800px] mx-auto space-y-6 animate-fade-in">
      <div className="pb-6 border-b border-white/[0.04]">
        <h1 className="text-xl font-bold text-white flex items-center gap-2">
          <Key className="text-amber-400" size={22} /> Manajemen Key Kriptografi
        </h1>
      ...
      <div className="glass-card p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-white font-bold text-[15px]">{user?.nama || user?.username || "—"}</p>
            <p className="text-slate-400 text-[12px]">{roleLabel[user?.role || ""] || user?.role}</p>
          </div>
        </div>
        ...
        <div>
          {loadingProfile ? (
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-bold text-[11px]">
              <CheckCircle size={13} /> Kunci Aktif &amp; Terverifikasi
            </div>
          ) : keyStatus === "pending" ? (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-300 font-bold text-[11px]">
              <Clock size={13} className="animate-spin" /> Menunggu Persetujuan Admin (CA)
            </div>
          ) : keyStatus === "rejected" ? (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-500/10 border border-red-500/20 text-red-300 font-bold text-[11px]">
              <XCircle size={13} /> Pengajuan Kunci Ditolak
            </div>
          ) : (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-800 border border-slate-700 text-slate-400 text-[11px]">
      ...
      {/* Notice jika Pending */}
      {keyStatus === "pending" && (
        <div className="glass-card p-5 border border-amber-500/20 bg-amber-500/5 flex items-start gap-3">
          <Clock size={18} className="text-amber-400 mt-0.5 shrink-0" />
          <div className="space-y-1">
            <p className="text-amber-300 font-bold text-[13px]">Permohonan Kunci Sedang Diverifikasi</p>
            <p className="text-[12px] text-slate-400 leading-relaxed">
              Kamu telah mengajukan Public Key baru. Untuk mencegah pembajakan akun dan penyamaran identitas, 
              kunci ini harus disetujui terlebih dahulu oleh <strong className="text-white">Administrator (Otoritas CA)</strong> sebelum 
      ...
      {/* Instruksi CA */}
      <div className="glass-card p-5 border border-blue-500/15 bg-blue-500/5">
        <div className="flex items-start gap-3">
          <Info size={16} className="text-blue-400 mt-0.5 shrink-0" />
          <div className="space-y-1.5">
            <p className="text-blue-300 font-bold text-[13px]">Prinsip Otoritas Kunci (Certificate Authority)</p>
            <ol className="text-[12px] text-slate-400 space-y-1 list-decimal list-inside">
              ...
              <li><strong className="text-amber-300">Download dan simpan Private Key (.pem)</strong> di laptop pribadi kamu (jangan sampai hilang)</li>
      ...
      <div className="glass-card p-7">
        <h3 className="text-white font-bold text-[15px] mb-4 flex items-center gap-2">
          <RefreshCw size={16} className="text-blue-400" /> Generate Key Pair Baru
        </h3>
        {keyStatus === "approved" && (
        ...
        <div className="space-y-4 animate-slide-up">
          ...
          <div className="glass-card p-6 border border-red-500/20 bg-red-500/5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-red-300 font-bold text-[14px] flex items-center gap-2">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={handleDownloadPrivateKey}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-500/20 border border-red-500/30 text-red-300 hover:bg-red-500/30 text-[12px] font-bold transition-colors">
                  <Download size={13} /> Download .pem
                </button>
              </div>
            </div>
            <div className="bg-black/40 rounded-xl p-4 border border-red-500/10">
              <pre className="font-mono text-[10px] text-red-300/80 break-all whitespace-pre-wrap">
                {showPrivKey ? generatedPrivateKey : generatedPrivateKey.replace(/[^\n]/g, "•")}
              </pre>
            </div>
            <p className="text-red-400/70 text-[11px] mt-2 font-medium">
          ...
          <div className="glass-card p-6 border border-emerald-500/20 bg-emerald-500/5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-emerald-300 font-bold text-[14px]">🔓 Public Key Baru (Siap Diajukan)</h3>
              <button onClick={() => handleCopyPublicKey(generatedPublicKey)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 hover:bg-emerald-500/30 text-[12px] font-bold transition-colors">
                <Copy size={13} /> {copied ? "Tersalin!" : "Salin"}
              </button>
            </div>
            <div className="bg-black/40 rounded-xl p-4 border border-emerald-500/10">
              <pre className="font-mono text-[10px] text-emerald-300/70 break-all whitespace-pre-wrap">{generatedPublicKey}</pre>
            </div>
      ...
      {currentPublicKey && !generatedPublicKey && keyStatus === "approved" && (
        <div className="glass-card p-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-slate-300 font-bold text-[14px]">Public Key Aktif &amp; Terverifikasi</h3>
            <button onClick={() => handleCopyPublicKey(currentPublicKey)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-700 text-slate-400 hover:text-white text-[12px] font-bold transition-colors">
              <Copy size={13} /> {copied ? "Tersalin!" : "Salin"}
            </button>
          </div>
          <div className="bg-black/40 rounded-xl p-4 border border-white/[0.04]">
            <pre className="font-mono text-[10px] text-slate-400 break-all whitespace-pre-wrap">{currentPublicKey}</pre>
          </div>
        </div>
      )}
    </div>
