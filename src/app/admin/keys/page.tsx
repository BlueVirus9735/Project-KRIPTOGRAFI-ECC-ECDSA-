  ...
  return (
    <div className="max-w-[1100px] mx-auto space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-card p-5 border border-amber-500/20 bg-amber-500/5 flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Menunggu Verifikasi</p>
            <h3 className="text-2xl font-black text-amber-400 mt-1">{pendingCount} Pengajuan</h3>
          </div>
        ...
        <div className="glass-card p-5 border border-emerald-500/20 bg-emerald-500/5 flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Kunci Aktif (Approved)</p>
            <h3 className="text-2xl font-black text-emerald-400 mt-1">{approvedCount} Pengguna</h3>
          </div>
        ...
        <div className="glass-card p-5 border border-blue-500/20 bg-blue-500/5 flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Total Terdaftar</p>
            <h3 className="text-2xl font-black text-blue-400 mt-1">{users.length} Akun</h3>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/[0.05] bg-slate-900/40">
                <th className="px-5 py-3.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Pengguna</th>
                <th className="px-5 py-3.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Peran (Role)</th>
                <th className="px-5 py-3.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Status Kunci</th>
                <th className="px-5 py-3.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Waktu Pengajuan</th>
                <th className="px-5 py-3.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider text-right">Aksi Administrator</th>
              </tr>
            </thead>
                ...
                filteredUsers.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-white/[0.01] transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <span className="px-2.5 py-1 rounded-md bg-slate-800 border border-slate-700 font-mono text-[11px] uppercase font-bold text-slate-300">
                          {u.role}
                        </span>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        {isPending && (
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-300 font-bold text-[11px]">
                            <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />
                            Pending Approval
                          </span>
                        )}
                        {isApproved && (
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 font-bold text-[11px]">
                            <CheckCircle size={12} className="text-emerald-400" />
                            Approved
                          </span>
                        )}
                        {isRejected && (
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-300 font-bold text-[11px]">
                            <XCircle size={12} className="text-red-400" />
                            Rejected
                          </span>
                        )}
                        {!isPending && !isApproved && !isRejected && (
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-500 text-[11px]">
                            Belum Ada Kunci
                          </span>
                        )}
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {isPending && (
                            <>
                              ...
                              <button
                                ...
                                disabled={actionLoading === u.id}
                                className="px-3 py-1.5 rounded-lg border border-red-500/30 text-red-400 hover:bg-red-500/10 font-bold text-[11px] transition-colors flex items-center gap-1 disabled:opacity-50"
                              >
                          ...
                          {isApproved && (
                            <button
                              ...
                              disabled={actionLoading === u.id}
                              className="px-2.5 py-1.5 rounded-lg border border-slate-700 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 text-[11px] font-semibold transition-colors flex items-center gap-1 disabled:opacity-50"
                            >
                              <Ban size={12} /> Cabut
                            </button>
                          )}
      ...
      {selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-[#0f172a] border border-slate-700 rounded-2xl p-6 w-full max-w-lg shadow-2xl animate-fade-in space-y-4">
            <div className="flex items-start justify-between border-b border-white/[0.04] pb-4">
              <div>
                <h3 className="text-white font-bold text-[15px]">Detail Public Key Pengguna</h3>
                <p className="text-slate-400 text-[12px]">{selectedUser.nama} (@{selectedUser.username}) • {selectedUser.role.toUpperCase()}</p>
              </div>
              <button onClick={() => setSelectedUser(null)} className="text-slate-500 hover:text-white">✕</button>
            </div>
            ...
            <div>
              ...
              <div className="bg-black/50 p-3.5 rounded-xl border border-white/[0.05]">
                <pre className="font-mono text-[10px] text-emerald-400/80 break-all whitespace-pre-wrap max-h-[180px] overflow-y-auto">
            ...
            {selectedUser.key_status === "pending" && (
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl border border-red-500/30 text-red-400 hover:bg-red-500/10 font-bold text-[12px]"
                >
                  Tolak Pengajuan
                </button>
                <button
