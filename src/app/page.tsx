  ...
  const cards = [
    {
      ...
      borderColor: "border-blue-200 dark:border-blue-500/15",
    },
    {
      label: "Database RPKH",
      value: stats.rpkh,
      ...
      borderColor: "border-amber-200 dark:border-amber-500/15",
    },
    {
      label: "Draf RTT",
      value: stats.draft,
      ...
      borderColor: "border-emerald-200 dark:border-emerald-500/15",
    },
    {
      label: "Validasi & Review",
      value: stats.review,
      icon: <Activity size={20} />,
      gradient: "from-purple-500/15 to-transparent",
      iconColor: "text-purple-600 dark:text-purple-400",
      borderColor: "border-purple-200 dark:border-purple-500/15",
    },
    {
      label: "Disahkan",
      value: stats.sah,
      ...
      borderColor: "border-rose-200 dark:border-rose-500/15",
    },
  ];
  ...
  return (
    <div className="space-y-8 max-w-full">
      ...
      <div className="flex items-center gap-3 flex-wrap animate-slide-down">
        <button className="glass-card py-2 px-4 flex items-center gap-3 cursor-pointer hover:border-slate-300 dark:hover:border-slate-600/30 transition-all text-[12px] font-semibold text-slate-700 dark:text-slate-300">
          Semua Dokumen
          <ChevronDown size={14} className="text-slate-500 dark:text-slate-600" />
        </button>
        <button className="glass-card py-2 px-4 flex items-center gap-3 cursor-pointer hover:border-slate-300 dark:hover:border-slate-600/30 transition-all text-[12px] font-semibold text-slate-700 dark:text-slate-300">
          Harian
          <ChevronDown size={14} className="text-slate-500 dark:text-slate-600" />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 stagger-children">
        {cards.map((card, i) => (
          <div
            key={i}
            className={`glass-card glass-card-hover bg-gradient-to-br ${card.gradient} border ${card.borderColor} p-5 flex flex-col justify-between h-[130px] group transition-all duration-300 cursor-default shadow-xs`}
          >
            <div className="flex justify-between items-start">
              ...
              <div
                className={`${card.iconColor} opacity-80 group-hover:opacity-100 transition-opacity`}
              >
                {card.icon}
              </div>
            </div>
      ...
      <div className="glass-card p-7 relative overflow-hidden animate-fade-in">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-[12px] text-slate-600 dark:text-slate-400 mt-1 font-medium">
              Penyusunan & pengelolaan sumber daya
            </p>
          </div>
          <button className="glass-card px-4 py-2 flex items-center gap-2 text-slate-700 dark:text-slate-300 cursor-pointer text-[12px] font-semibold hover:border-slate-300 dark:hover:border-slate-600/30 transition-all">
            1 bulan terakhir
            <ChevronDown size={14} />
          </button>
        </div>
        ...
        <div className="h-[280px] w-full relative z-10">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart
              data={dynamicChart.length > 0 ? dynamicChart : undefined}
            ...
            >
              ...
              <CartesianGrid
                strokeDasharray="3 3"
                vertical={false}
                stroke={isDark ? "rgba(148,163,184,0.06)" : "rgba(148,163,184,0.25)"}
              />
              <XAxis
                dataKey="name"
                axisLine={false}
                tickLine={false}
                tick={{ fill: isDark ? "#64748b" : "#475569", fontSize: 11, fontWeight: 500 }}
                dy={10}
              />
              <YAxis hide />
              <Tooltip
        ...
        <div className="flex justify-center gap-8 mt-6 border-t border-slate-200 dark:border-white/[0.04] pt-5">
          {[
